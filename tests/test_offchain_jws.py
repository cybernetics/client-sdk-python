...
def test_serialize_deserialize():
    ...
    resp = offchain.jws.deserialize(ret, offchain.CommandResponseObject, account.private_key.public_key().verify)
    assert resp == response


def test_deserialize_error_if_not_3_parts():
    with pytest.raises(ValueError):
        offchain.jws.deserialize(b".".join([b"header", b"payload"]), offchain.CommandResponseObject, lambda: None)


def test_deserialize_error_for_mismatched_protected_header():
    with pytest.raises(ValueError):
        offchain.jws.deserialize(
            b".".join([b"header", b"payload", b"sig"]), offchain.CommandResponseObject, lambda: None
        )
...
def test_deserialize_error_for_invalid_signature():
    ...
    with pytest.raises(cryptography.exceptions.InvalidSignature):
        resp = offchain.jws.deserialize(data, offchain.CommandResponseObject, account2.private_key.public_key().verify)
