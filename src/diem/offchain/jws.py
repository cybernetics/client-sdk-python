...
def serialize(
    obj: typing.Union[CommandRequestObject, CommandResponseObject], sign: typing.Callable[[bytes], bytes]
) -> bytes:
    return serialize_string(to_json(obj), sign)


def deserialize(msg: bytes, klass: typing.Type[T], verify: typing.Callable[[bytes, bytes], None]) -> T:
    decoded_body, sig, signing_msg = deserialize_string(msg)
    verify(sig, signing_msg)
    try:
        obj = from_json(decoded_body, klass)
    except ValidationError as e:
        raise ValueError(
            f"invalid JWS message body, could not deserialize it into {klass}: {e}\ndecoded JWS body: {decoded_body}"
        ) from e
...
def deserialize_string(msg: bytes) -> typing.Tuple[str, bytes, bytes]:
    ...
    return (decode(body_bytes).decode(ENCODING), decode(sig.encode(ENCODING)), signing_message(body_bytes))
